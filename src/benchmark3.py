...
for i in range(100):
  ...
  rf1 = RandomForestClassifier(n_estimators=600, 
    n_jobs=-1, 
    criterion="entropy", 
    max_depth=10, 
    max_features=6, 
  ...
  AUC1 = roc_auc_truncated(train_eval['signal'], train_eval_probs1)
  print ('AUC UB', AUC1)

  train_eval_probs2 = gbm.predict(xgb.DMatrix(train_eval[features]))
  AUC2 = roc_auc_truncated(train_eval['signal'], train_eval_probs2)
  print ('AUC XGB', AUC2)

  train_eval_probs3 = rf1.predict_proba(train_eval[features])[:,1]
  AUC3 = roc_auc_truncated(train_eval['signal'], train_eval_probs3)
  print ('AUC UB', AUC3)
  ...
  submission.to_csv("predictions3/benchmark3_{timestamp}.csv".format(timestamp=time.time()), index=False)
